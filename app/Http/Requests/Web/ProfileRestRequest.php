...
class ProfileRestRequest extends FormRequest
{
    ...
    {
       $userId = optional($this->user('restaurant'))->id;

       $rules = [
        'name' => 'required|min:3',
        'email' => ['required', 'email', Rule::unique('restaurants', 'email')->ignore($userId)],
        'phone' => ['required', 'regex:/^\+[1-9][0-9]{7,15}$/'],
        'old_password' => ['nullable', function ($attribute, $value, $fail) {
            if ($this->filled('old_password') && !Hash::check($value, $this->user('restaurant')->password)) {
    ...
    public function passedValidation()
    {
        if ($this->filled('old_password') && $this->filled('password')) {
            ...
            $this->merge(['password' => $this->user('restaurant')->password]);
        }
    }
}
